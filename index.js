...
// const uri =
//   "mongodb+srv://<username>:<password>@cluster0.xicfz.mongodb.net/?retryWrites=true&w=majority";
...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.xicfz.mongodb.net/?retryWrites=true&w=majority`;
const client = new MongoClient(uri, {
  useNewUrlParser: true,
  useUnifiedTopology: true,
  serverApi: ServerApiVersion.v1,
...
async function run() {
  try {
    ...
    const toolsCollection = client.db("powertools").collection("tools");
    const ordersCollection = client.db("powertools").collection("orderDetails");
    const reviewCollection = client.db("powertools").collection("review");
...
app.get("/", (req, res) => {
  res.send("welcome to power tools website");
});

app.listen(port, () => {
  console.log(`power tools website listening on port ${port}`);
});
